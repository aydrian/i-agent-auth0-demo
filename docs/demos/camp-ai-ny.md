...
| 3:45–4:30 | Wrap | Reopen chat, send *"hi"*. Agent surfaces the order-confirmation card via the unacknowledged-purchase path. | "The chat agent knows the autonomous one acted because the unacknowledged-purchase count is in its system prompt. Three Auth0 features stacked: identity-aware system prompt, Token Vault for delegated standing access, CIBA for delegated single-action approval. One control plane. The agent owned the decision; I owned the consent." |
