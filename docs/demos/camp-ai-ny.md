...
| 2:30–4:15 | **CIBA** | Send *"watch the Mac Mini M4 and buy it if it drops below $799 — I want one for my Exo cluster."* Agent confirms the watch. Pivot away from chat ("the agent runs on its own from here"). Open <http://localhost:8000/admin>, drop Mac Mini to $749 for 60 minutes. In a terminal: `pnpm demo:camp-ai-ny trigger`. Phone buzzes. Read the **agent-composed** binding message aloud — emphasize the agent wrote it, not a template. Approve. | "The watch row stores my intent in plain English. A cron tick wakes an LLM agent — same one I just talked to, but headless — that pulls current price plus history, decides whether my intent is satisfied, and *only then* fires CIBA. Auth0 routes a Guardian push to my phone with a binding message the agent itself composed. I approve, the agent gets a `product:buy`-scoped token for one call, and the purchase happens. If the agent had decided not to act, I'd never have been bothered." |
